...
setmetatable(Menu, {
    ...
    __newindex = function(self, Key, tbl)
        __Menu[Key] = tbl
...
local function SetDraggable(self: GuiObject) -- who ever thought using dis method is stupid
    table.insert(Draggables, self)
    local DragOrigin
    local GuiOrigin
    local connection

    self.InputBegan:Connect(function(Input: InputObject)
        if (not Dragging.Gui and not Dragging.True) and 
    ...
    connection = UserInput.InputChanged:Connect(function(Input: InputObject)
        if Dragging.Gui ~= self then return end
        if not (UserInput:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or UserInput:IsMouseButtonPressed(Enum.UserInputType.Touch)) then
        ...
        if Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch then
            ...
            local OffsetY = math.clamp(GuiOrigin.Y.Offset + Delta.Y + ScaleY, 0, ScreenSize.Y - self.AbsoluteSize.Y)
...
local function SetFixedDraggable(Instance)
    if not Instance:IsA("GuiObject") then
        warn("SetFixedDraggable can only be used with GuiObjects")
        return
    end
    ...
    Instance.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            -- Connect InputChanged only once
            local connection
            connection = UserInput.InputChanged:Connect(function(changedInput)
...
Menu_Frame.BorderColor3 = Color3.fromRGB(56, 56, 56)
...
function Menu.TextBox(Tab_Name: string, Container_Name: string, Name: string, tbl: string, Callback: any, ToolTip: string): TextBox
    local Container = GetContainer(Tab_Name, Container_Name)
    ...
    TextBox.tbl = typeof(tbl) == "string" and tbl or ""
    ...
    function TextBox:Gettbl(): string
        return self.tbl
    end

    function TextBox:Settbl(tbl: string)
        self.tbl = tostring(tbl)
        GuiTextBox.Text = self.tbl
    end
...
function Menu.CheckBox(Tab_Name: string, Container_Name: string, Name: string, Boolean: boolean, Callback: any, ToolTip: string): CheckBox
    ...
    CheckBox.tbl = typeof(Boolean) == "boolean" and Boolean or false
    CheckBox.Callback = typeof(Callback) == "function" and Callback or function() end

    function CheckBox:Update(tbl: boolean)
        self.tbl = typeof(tbl) == "boolean" and tbl
        Button.BackgroundColor3 = self.tbl and Menu.Accent or Menu.ItemColor
    end
    ...
    function CheckBox:Gettbl(): boolean
        return self.tbl
    end

    function CheckBox:Settbl(tbl: boolean)
        self:Update(tbl)
    end
    ...
    Button.Parent = Label
    AddEventListener(Button, function()
        Button.BackgroundColor3 = CheckBox.tbl and Menu.Accent or Menu.ItemColor
    end)
    Button.MouseButton1Click:Connect(function()
        CheckBox:Update(not CheckBox.tbl)
        CheckBox.Callback(CheckBox.tbl)
    end)

    CheckBox:Update(CheckBox.tbl)
    Container:UpdateSize(20)
...
function Menu.Hotkey(Tab_Name: string, Container_Name: string, Name: string, Key:EnumItem, Callback: any, ToolTip: string): Hotkey
    ...
    function Hotkey:Gettbl(): EnumItem--, string
        return self.Key, self.Mode
    end

    function Hotkey:Settbl(Key: EnumItem, Mode: string)
...
function Menu.Slider(Tab_Name: string, Container_Name: string, Name: string, Min: number, Max: number, tbl: number, Unit: string, Scale: number, Callback: any, ToolTip: string): Slider
    local Container = GetContainer(Tab_Name, Container_Name)
    local Label = CreateLabel(Container.self, "Slider", Name, UDim2.new(1, -10, 0, 15), UDim2.fromOffset(20, Container:GetHeight()))
    local Button = Instance.new("TextButton")
    local tblBar = Instance.new("TextLabel")
    local tblBox = Instance.new("TextBox")
    local tblLabel = Instance.new("TextLabel")
    ...
    Slider.tbl = typeof(tbl) == "number" and tbl or 100
    ...
    local function UpdateSlider(Percentage: number)
        ...
        local tbl = Slider.Min + ((Slider.Max - Slider.Min) * Percentage)
        local Scale = (10 ^ Slider.Scale)
        Slider.tbl = math.round(tbl * Scale) / Scale

        tblBar.Size = UDim2.new(Percentage, 0, 0, 5)
        tblBox.Text = "[" .. Slider.tbl .. "]"
        tblLabel.Text = Slider.tbl .. Slider.Unit
    end
    ...
    function Slider:Gettbl(): number
        return self.tbl
    end

    function Slider:Settbl(tbl: number)
        self.tbl = typeof(tbl) == "number" and math.clamp(tbl, self.Min, self.Max) or self.Min
        local Percentage = (self.tbl - self.Min) / (self.Max - self.Min)
    ...
    tblBar.Name = "tblBar"
    tblBar.BackgroundColor3 = Menu.Accent
    tblBar.BorderSizePixel = 0
    tblBar.Size = UDim2.fromScale(1, 1)
    tblBar.Text = ""
    tblBar.Parent = Button
    AddEventListener(tblBar, function()
        tblBar.BackgroundColor3 = Menu.Accent
    end)
    
    tblBox.Name = "tblBox"
    tblBox.BackgroundTransparency = 1
    tblBox.Position = UDim2.new(1, -65, 0, 5)
    tblBox.Size = UDim2.fromOffset(50, 10)
    tblBox.Font = Enum.Font.SourceSans
    tblBox.Text = ""
    tblBox.TextColor3 = Color3.new(1, 1, 1)
    tblBox.TextSize = 12
    tblBox.TextXAlignment = Enum.TextXAlignment.Right
    tblBox.ClipsDescendants = true
    tblBox.Parent = Label
    tblBox.FocusLost:Connect(function()
        Slider.tbl = tonumber(tblBox.Text) or 0
        local Percentage = (Slider.tbl - Slider.Min) / (Slider.Max - Slider.Min)
        Slider:Update(Percentage)
        Slider.Callback(Slider.tbl)
    end)

    tblLabel.Name = "tblLabel"
    tblLabel.BackgroundTransparency = 1
    tblLabel.Position = UDim2.new(1, 0, 0, 2)
    tblLabel.Size = UDim2.new(0, 0, 1, 0)
    tblLabel.Font = Enum.Font.SourceSans
    tblLabel.Text = ""
    tblLabel.TextColor3 = Color3.new(1, 1, 1)
    tblLabel.TextSize = 14
    tblLabel.Parent = tblBar

	Button.InputBegan:Connect(function(Input, Process)
	    if not Dragging.Gui and not Dragging.True and
	       (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) then
	        local InputPosition = Vector2.new(Input.Position.X, Input.Position.Y)
	        if Button.AbsolutePosition.X <= InputPosition.X and
	           Button.AbsolutePosition.Y <= InputPosition.Y and
	           InputPosition.X <= Button.AbsolutePosition.X + Button.AbsoluteSize.X and
	           InputPosition.Y <= Button.AbsolutePosition.Y + Button.AbsoluteSize.Y then
	...
	UserInput.InputChanged:Connect(function(Input, Process)
	    if Dragging.Gui ~= Button or not Dragging.True then return end
...
function Menu.ColorPicker(Tab_Name: string, Container_Name: string, Name: string, Color: Color3, Alpha: number, Callback: any, ToolTip: string): ColorPicker
    ...
    function ColorPicker:Settbl(Color: Color3, Alpha: number)
    ...
    function ColorPicker:Gettbl(): (Color3, number)
    ...
    HexBox.FocusLost:Connect(function()
        pcall(function()
            local Color = string.sub(HexBox.Text, 1, 7)
            local Alpha = string.sub(HexBox.Text, 8, #HexBox.Text)
    ...
    AlphaButton.Name = "Alpha"
    AlphaButton.BorderColor3 = Menu.BorderColor
    AlphaButton.Position = UDim2.new(0, 175, 0, 4)
    AlphaButton.Size = UDim2.new(0, 10, 0, 140)
    AlphaButton.Image = "rbxassetid://9090739505"
    AlphaButton.ScaleType = Enum.ScaleType.Crop
    AlphaButton.AutoButtonColor = false
    AlphaButton.Parent = Selected_ColorPicker
    CreateStroke(AlphaButton, Color3.new(), 1)
    AddEventListener(AlphaButton, function()
        AlphaButton.BorderColor3 = Menu.BorderColor
    end)
    ...
    AlphaCursor.Parent = AlphaButton
    ...
    AlphaColorGradient.Parent = AlphaButton

    local function UpdateSaturation(PercentageX: number, PercentageY: number)
        PercentageX = math.clamp(PercentageX, 0, 1)
        PercentageY = math.clamp(PercentageY, 0, 1)
    ...
    AlphaButton.InputBegan:Connect(function(Input: InputObject, Process: boolean)
        if (not Dragging.Gui and not Dragging.True) and (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) then
            Dragging = {Gui = AlphaButton, True = true}
            local InputPosition = Vector2.new(Input.Position.X, Input.Position.Y)
            local Percentage = (InputPosition - AlphaButton.AbsolutePosition) / AlphaButton.AbsoluteSize
    ...
    UserInput.InputChanged:Connect(function(Input: InputObject, Process: boolean)
        if not (Dragging.Gui == Saturation or Dragging.Gui == AlphaButton or Dragging.Gui == Hue) then return end
        if not (UserInput:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or UserInput:IsMouseButtonPressed(Enum.UserInputType.Touch)) then
            Dragging = {Gui = nil, True = false}
            return
        end
        ...
        if Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch then
            ...
            if Dragging.Gui == AlphaButton then
                local Percentage = (InputPosition - AlphaButton.AbsolutePosition) / AlphaButton.AbsoluteSize
                UpdateAlpha(Percentage.Y)
            end
...
function Menu.ComboBox(Tab_Name: string, Container_Name: string, Name: string, tbl: string, tbl_Items: table, Callback: any, ToolTip: string): ComboBox
    local Container = GetContainer(Tab_Name, Container_Name)
    ...
    ComboBox.tbl = typeof(tbl) == "string" and tbl or ""
    ComboBox.Items = typeof(tbl_Items) == "table" and tbl_Items or {}

    local function Updatetbl(tbl: string)
        ComboBox.tbl = tostring(tbl)
        Button.Text = ComboBox.tbl or "[...]"
    end
    ...
    local function AddItem(Name: string)
        ...
        Button.TextColor3 = ComboBox.tbl == Button.Text and Menu.Accent or Color3.new(1, 1, 1)
        Button.TextSize = 14
        ...
        Button.MouseButton1Click:Connect(function()
            ...
            Updatetbl(Button.Text)
            UpdateSelected()
            ComboBox.Callback(ComboBox.tbl)
        end)
        AddEventListener(Button, function()
            Button.BackgroundColor3 = Menu.ItemColor
            if ComboBox.tbl == Button.Text then
                Button.TextColor3 = Menu.Accent
            else
                Button.TextColor3 = Color3.new(1, 1, 1)
            end
    ...
    function ComboBox:Update(tbl: string, Items: any)
        Updatetbl(tbl)
        if typeof(Items) == "table" then
            ...
            for _, Button in ipairs(ItemObjects) do
                Button.TextColor3 = self.tbl == Button.Text and Menu.Accent or Color3.new(1, 1, 1)
            end
        end
    end
    ...
    function ComboBox:Gettbl(): table
        return self.tbl
    end

    function ComboBox:Settbl(tbl: string, Items: any)
        if typeof(Items) == "table" then
            self.Items = Items
        end
        self:Update(tbl, self.Items)
    end
    ...
    ComboBox:Update(ComboBox.tbl, ComboBox.Items)
...
function Menu.MultiSelect(Tab_Name: string, Container_Name: string, Name: string, tbl_Items: table, Callback: any, ToolTip: string): MultiSelect
    local Container = GetContainer(Tab_Name, Container_Name)
    ...
    MultiSelect.Items = typeof(tbl_Items) == "table" and tbl_Items or {}
    MultiSelect.tbl = {}
    ...
    local function Updatetbl()
        MultiSelect.tbl = GetSelectedItems()
        Button.Text = #MultiSelect.tbl > 0 and table.concat(MultiSelect.tbl, ", ") or "[...]"
    end
    ...
    local function AddItem(Name: string, Checked: boolean)
        ...
        Button.MouseButton1Click:Connect(function()
            ...
            Updatetbl()
            MultiSelect.Callback(MultiSelect.Items) -- don't send tbl
        end)
        AddEventListener(Button, function()
            Button.BackgroundColor3 = Menu.ItemColor
    ...
    function MultiSelect:Update(tbl: any)
    ...
    function MultiSelect:Gettbl(): table
        return self.Items
    end

    function MultiSelect:Settbl(tbl: any)
        self:Update(tbl)
    end
...
function Menu.ListBox(Tab_Name: string, Container_Name: string, Name: string, Multi: boolean, tbl_Items: table, Callback: any, ToolTip: string): ListBox
    local Container = GetContainer(Tab_Name, Container_Name)
    local List = Instance.new("ScrollingFrame")
    local ListLayout = Instance.new("UIListLayout")
    ...
    ListBox.Items = typeof(tbl_Items) == "table" and tbl_Items or {}
    ListBox.tbl = {}
    ...
    local function Updatetbl(tbl: any)
        if ListBox.Method == "Default" then
            ListBox.tbl = tostring(tbl)
        else
            ListBox.tbl = GetSelectedItems()
        end
    end
    ...
    local function AddItem(Name: string, Checked: boolean)
        ...
        Button.Parent = List
        if ListBox.Method == "Default" then
            Button.TextColor3 = ListBox.tbl == Button.Text and Menu.Accent or Color3.new(1, 1, 1)
            Button.MouseButton1Click:Connect(function()
                for _, v in ipairs(List:GetChildren()) do
                    if v:IsA("GuiButton") then
                        if v == Button then continue end
                ...
                Updatetbl(Button.Text)
                UpdateSelected()
                ListBox.Callback(ListBox.tbl)
            end)
            AddEventListener(Button, function()
                Button.BackgroundColor3 = Menu.ItemColor
                if ListBox.tbl == Button.Text then
                    Button.TextColor3 = Menu.Accent
                else
                    Button.TextColor3 = Color3.new(1, 1, 1)
                end
            ...
            AddEventListener(Button, function()
                Button.BackgroundColor3 = Menu.ItemColor
                if table.find(ListBox.tbl, Name) then
                    Button.TextColor3 = Menu.Accent
                else
                    Button.TextColor3 = Color3.new(1, 1, 1)
                end
    ...
    function ListBox:Update(tbl: string, Items: any)
    ...
    function ListBox:Settbl(tbl: string, Items: any)
        if self.Method == "Default" then
            if typeof(Items) == "table" then
                self.Items = Items
            end
    ...
    if ListBox.Method == "Default" then
        ListBox:Update(ListBox.tbl, ListBox.Items)
    else
...
function Menu.Indicators(): Indicators
    ...
    function Indicators.Add(Name: string, Type: string, tbl: string, ...): Indicator
        ...
        if Type == "Bar" then
            ...
            tblLabel.Name = "tbl"
            tblLabel.BorderSizePixel = 0
            tblLabel.BackgroundColor3 = Menu.Accent
            tblLabel.Text = ""
            tblLabel.Parent = ObjectBase
            AddEventListener(tblLabel, function()
                tblLabel.BackgroundColor3 = Menu.Accent
            end)
        else
            Object.Size = UDim2.new(0, 230, 0, 15)
        ...
        function Indicator:Update(tbl: string, ...)
            if Indicators.List[Name] then
                if Type == "Text" then
                    self.tbl = tbl
                    Object.State.Text = tbl
                elseif Type == "Bar" then
                    local Min, Max = select(1, ...)
                    self.Min = typeof(Min) == "number" and Min or self.Min
                    self.Max = typeof(Max) == "number" and Max or self.Max

                    local Scale = (self.tbl - self.Min) / (self.Max - self.Min)
                    Object.State.Text = "[" .. tostring(self.tbl) .. "]"
                    Object.Bar.tbl.Size = UDim2.new(math.clamp(Scale, 0, 1), 0, 0, 5)
                end
                self.tbl = tbl
            end
        end
        ...
        Indicator:Update(Indicator.tbl, ...)
...
function Menu.Watermark(): Watermark
    ...
    Watermark.Frame.BackgroundColor3 = Color3.fromRGB(16, 16, 16)
    ...
    SetFixedDraggable(Watermark.Frame)
    ...
    Watermark.Title.Font = Enum.Font.SourceSans
