...
local function queue_property_set(object, property, value)
	if object.AbsoluteVisible then
		...
		if not success then
			warn("Failed to set property '" .. property .. "' on object: " .. tostring(err))
		end
	else
...
function Render:UpdateQueue()
	for property, value in next, self._queue do
		...
		if self._outline and (property == "Position" or property == "Size" or property == "ZIndex") then
			local successOutline, errOutline = pcall(function()
				setproperty(self._outline, property, 
					property == "ZIndex" and value - 1
					or property == "Size" and value + Vector2.new(2, 2)
					or property == "Position" and value - Vector2.new(1, 1)
				)
			end)

			if not successOutline then
				warn("Failed to set outline property '" .. property .. "': " .. tostring(errOutline))
			end
		end
