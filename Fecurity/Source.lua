...
do
	...
	local ProggyTiny = Register_Font("adwdawdwadadwadawdawdawdawd!", 100, "Normal", {
		...
		Font = game:HttpGet("https://raw.githubusercontent.com/NymeraAnHomie/Fonts/main/ProggyTiny.ttf"),
	})
...
function library:draggify(frame)
	...
	library:connection(uis.InputChanged, function(input)
		if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			...
			local newX = clamp(start_size.X.Offset + (input.Position.X - start.X), 0, viewport.X - frame.Size.X.Offset)
			local newY = clamp(start_size.Y.Offset + (input.Position.Y - start.Y), 0, viewport.Y - frame.Size.Y.Offset)

			frame.Position = dim2(0, newX, 0, newY)
		end
	end)
end
...
function library:slider(options)
	...
	library:connection(uis.InputChanged, function(input)
		if cfg.dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			local sliderAbsPos = items["slider"].AbsolutePosition
			local sliderAbsSize = items["slider"].AbsoluteSize
	
			local relativeX = clamp((input.Position.X - sliderAbsPos.X) / sliderAbsSize.X, 0, 1)
			local value = cfg.min + (cfg.max - cfg.min) * relativeX
...
function library:dropdown(options)
	...
	items["text_holder"].InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
...
function library:colorpicker(options)
	...
	items["alpha_slider"].InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			dragging_alpha = true
		end
	end)
	
	items["hue_slider"].InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			dragging_hue = true
		end
	end)
	
	items["color_saturation"].InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			dragging_sat = true
		end
	end)
