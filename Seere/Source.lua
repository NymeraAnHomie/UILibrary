...
function library:addTab(name)
    ...
    function tab:createGroup(pos,groupname) -- newTab[pos == 0 and "left" or "right"] 
        ...
        title.BackgroundTransparency = 0 --1.000
        ...
        function group:addToggle(args)
            ...
            local state = args.default or args.state
            ...
            function toggle:addColorpicker(args)
                ...
                hue.MouseEnter:Connect(function()
                    local input = hue.InputBegan:connect(function(key)
                        if key.UserInputType == Enum.UserInputType.MouseButton1 and key.UserInputType == Enum.UserInputType.Touch then
        ...
        function group:addSlider(args, sub)
		    ...
		    local increment = args.increment or 1  -- Default increment
		    local decimalPlaces = tostring(increment):find("%.") and #tostring(increment):split(".")[2] or 0
		    ...
		    local function updateValue(value)
		        ...
		        if roundedValue ~= 0 then
		            fill:TweenSize(UDim2.new((roundedValue - args.min) / (args.max - args.min), 0, 1, 0), Enum.EasingDirection.In, Enum.EasingStyle.Sine, 0.01)
		        else
		            fill:TweenSize(UDim2.new(0, 1, 1, 0), Enum.EasingDirection.In, Enum.EasingStyle.Sine, 0.01)
		        end
		        valuetext.Text = string.format("%." .. decimalPlaces .. "f", roundedValue) .. sub
		        library.flags[args.flag] = roundedValue
		        if args.callback then
		            args.callback(roundedValue)
		        end
		    end
...
function library:createConfig()
    ...
    writefile("Xiro/"..name..".cfg",game:GetService("HttpService"):JSONEncode(jig))
...
function library:loadConfig()
    local name = library.flags["selected_config"]
    if not isfile("Xiro/"..name..".cfg") then
        library:notify("Config file not found!")
        return
    end
    local config = game:GetService("HttpService"):JSONDecode(readfile("Xiro/"..name..".cfg"))
...
function library:refreshConfigs()
    local tbl = {}
    for i,v in next, listfiles("Xiro") do
        table.insert(tbl,v)
    end
...
function library:deleteConfig()
    if isfile("Xiro/"..library.flags["selected_config"]..".cfg") then
        delfile("Xiro/"..library.flags["selected_config"]..".cfg")
        library:refreshConfigs()
    end
end
